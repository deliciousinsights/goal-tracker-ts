...
// été ajoutés à l’historique.  Par ailleurs, lorsque l’application se lance, si
// la journée “en cours” dans l’état global est désormais passée, une
// historisation est immédiatement déclenchée.
//
// C’est en fait le *reducer* consolidé qui fait tout le boulot côté données :
// ce module est surtout chargé de déclencher (`store.dispatch(…)`) la bonne
...
if (module.hot) {
  module.hot.accept()
  // Le HMR sur ce module doit prendre soin d’annuler le *timer* de vérification
  // en vigueur, puisque l’initialisation de la nouvelle version du module va en
  // ré-inscrire un tout de suite.
  module.hot.dispose(() => {
    clearInterval(clock)
  })
}
